#!/bin/bash
#SBATCH --job-name=validate_timepoint_vc
#SBATCH --output=validate_timepoint_vc_%j.out
#SBATCH --error=validate_timepoint_vc_%j.err
#SBATCH --ntasks=1
...
TIMEPOINT="${1:?Usage: sbatch scripts/validation/validate_timepoint_variant_calling.sh t2}"
...
if [ "${#ACTIVE_CLONES[@]}" -eq 0 ]; then
  echo "No active clones found in manifest: $MANIFEST" >&2
  exit 2
fi

TRUTH_VCFS=()
for clone in "${ACTIVE_CLONES[@]}"; do
  truth_vcf="$PATIENT_DIR/prepared_hg38_${TIMEPOINT}/hg38_vcf/${clone}.hg38.normalized.vcf"
  test -s "$truth_vcf"
  TRUTH_VCFS+=("$truth_vcf")
done

echo "=== Active clone truth VCFs ==="
for f in "${TRUTH_VCFS[@]}"; do
  ls -lh "$f"
done
...
find_reference() {
  ...
  local candidates=(
    "$PATIENT_DIR/prepared_hg38_${TIMEPOINT}/ref_cache/hg38/hg38.fa"
    "$PATIENT_DIR/prepared_hg38_${TIMEPOINT}/ref_cache/hg38/hg38.fa.gz"
...
Set HG38_REF when submitting, for example:
  sbatch --export=ALL,HG38_REF=/path/to/hg38.fa scripts/validation/validate_timepoint_variant_calling.sh t2
...
} | bcftools sort -Oz -o "$OUT_DIR/${TIMEPOINT}_truth.raw.vcf.gz"

bcftools norm -d exact -f "$REF" \
  -Oz -o "$OUT_DIR/${TIMEPOINT}_truth.vcf.gz" \
  "$OUT_DIR/${TIMEPOINT}_truth.raw.vcf.gz"
bcftools index -f "$OUT_DIR/${TIMEPOINT}_truth.vcf.gz"

bcftools query -f '%CHROM\t%POS0\t%END\n' "$OUT_DIR/${TIMEPOINT}_truth.vcf.gz" \
  | awk 'BEGIN{OFS="\t"} {s=$2-20; if(s<0)s=0; print $1,s,$3+20}' \
  | sort -k1,1 -k2,2n \
  > "$OUT_DIR/${TIMEPOINT}_truth.padded.bed"

echo "Truth variants:"
bcftools view -H "$OUT_DIR/${TIMEPOINT}_truth.vcf.gz" | wc -l
...
run_validation() {
  ...
  echo "=== BAM checks ==="
  samtools quickcheck -v "$OUT_DIR/normal.bam" "$OUT_DIR/tumor_${TIMEPOINT}.bam"
  samtools flagstat -@ "$INDEX_THREADS" "$OUT_DIR/normal.bam" > "$OUT_DIR/normal.flagstat.txt"
  samtools flagstat -@ "$INDEX_THREADS" "$OUT_DIR/tumor_${TIMEPOINT}.bam" > "$OUT_DIR/tumor_${TIMEPOINT}.flagstat.txt"

  echo "=== Mutect2 restricted to ${TIMEPOINT} truth regions ==="
  gatk Mutect2 \
    -R "$REF" \
    -I "$OUT_DIR/tumor_${TIMEPOINT}.bam" \
    -tumor "tumor_${TIMEPOINT}" \
    -I "$OUT_DIR/normal.bam" \
    -normal normal \
    -L "$OUT_DIR/${TIMEPOINT}_truth.padded.bed" \
    -O "$OUT_DIR/${TIMEPOINT}.mutect2.unfiltered.vcf.gz"

  gatk FilterMutectCalls \
    -R "$REF" \
    -V "$OUT_DIR/${TIMEPOINT}.mutect2.unfiltered.vcf.gz" \
    -O "$OUT_DIR/${TIMEPOINT}.mutect2.filtered.vcf.gz"

  echo "=== Normalize PASS calls and compare to truth ==="
  bcftools view -f PASS "$OUT_DIR/${TIMEPOINT}.mutect2.filtered.vcf.gz" \
    | bcftools norm -f "$REF" -d exact -Oz \
    -o "$OUT_DIR/${TIMEPOINT}.called.pass.norm.vcf.gz"
  bcftools index -f "$OUT_DIR/${TIMEPOINT}.called.pass.norm.vcf.gz"

  rm -rf "$OUT_DIR/isec_truth_vs_called"
  bcftools isec \
    -p "$OUT_DIR/isec_truth_vs_called" \
    "$OUT_DIR/${TIMEPOINT}_truth.vcf.gz" \
    "$OUT_DIR/${TIMEPOINT}.called.pass.norm.vcf.gz"
...
END_EPOCH=$(date +%s)
{
  ...
  echo "truth_clones=${ACTIVE_CLONES[*]}"
  echo "=== output size ==="
